...
public:
     ...
     * \return DroiUser instance.
     */
    static DroiUser* login( const std::string& userId, const std::string& password, DroiError* error );

    /**
     *  Login user with anonymous type.
     * \param error Pass DroiError to get error code. Pass null to ignore error details.
     */
    static DroiUser* loginWithAnonymous( DroiError* error );
...
private:
    ...
    static void cleanUp();
    static RefPtr<DroiUser> g_currentUser;
    ValueMap mSession;
