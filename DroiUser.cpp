...
DroiUser* DroiUser::g_currentUser = nullptr;
...
RefPtr<DroiUser> DroiUser::login( const std::string& userId, const std::string& password, DroiError* error )
{
...
RefPtr<DroiUser> DroiUser::loginWithAnonymous( DroiError* err )
{
    DroiUser::cleanUp();
    std::string deviceId = DroiCore::getDeviceId();
    std::string installationId = DroiCore::getInstallationId();
