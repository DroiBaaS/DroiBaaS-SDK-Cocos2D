...
void DroiObject::registerCustomClass(std::string classname, ::createObject factory)
{
    classFactory::const_iterator iter =  registerdClasses.find(classname);
    if (iter != registerdClasses.end()) {
        log("%s was registed already", classname.c_str());
        return;
    }

    registerdClasses.insert(std::make_pair(classname, factory));
}
